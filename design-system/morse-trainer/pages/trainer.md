...
- **The first session introduces before it tests.** Both starting characters get an intro card, then a `K M M K` contrast run and an `M K` repair pair, before the policy takes over. A scheduler with no data has nothing to schedule. That opening run *is* arrive and repair, so the first sitting is the same twenty rounds as any other.
