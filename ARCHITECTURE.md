...
**Learning gates the world.** `src/data/transmissions.js` is a pure catalog of twelve messages from stations that are not us. Each carries the prefix of `KOCH_ORDER` it needs, and the catalog is ordered by that prefix, so a transmission can only arrive once every character inside it has genuinely been taught — the world opens as the ear does, and nothing about it is a reward currency. `availableTransmissions(unlocked, archive)` returns what could come through right now, in catalog order; `nextTransmission` in `progress.js` adds the two live conditions — at most one per session, and never while `newPaused` is on, because a learner already behind on reviews has not earned a new place to look. It is served in the **use** phase as a `signal` round, which is a Word round in every respect that matters: positional typing with Backspace, `context`-only scoring, and no contact with the character scheduler. The word gaps in a message are heard rather than typed; the cursor steps over them.

A transmission is archived by being **decoded**, never by being unlocked. `progress.archive[id] = { ts, independent }` records when, and whether the learner needed a replay or a hint — help still archives the message, because it was still read, but only an unaided decode is independent. A miss archives nothing and the message returns in a later session. The archive is a section of the letters drawer, not a screen, and the note each message carries is spoken once, on the session-end card, in the session it arrived.

**Transmissions arrive over static.** `MorseAudio.playText(text, speed, { ambience })` lays a bandpass-filtered noise bed (950 Hz, Q 0.5, gain 0.012) under the tones, from 50 ms before the first mark to 250 ms after the last. It is scheduled through the same node list as the oscillators, so the overlap guard and `stop()` take it with everything else, and it is deliberately excluded from `isPlaying()` — the answer window still opens at the last tone. Only the incoming-transmission card and unknown-transmission rounds ask for it; letter drills, Sprint, Send and the placement trial never do. There is no setting for it: it is part of what a transmission *is*, not a preference.
